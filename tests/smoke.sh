...
chmod a-x "$EXECBIT_DST"
if [ -x "$EXECBIT_DST" ]; then
...
if [ -x "$EXECBIT_DST" ]; then
  pass "s3b: session-start.sh executable again after mode-only --update"
else
  fail "s3b: session-start.sh executable again after mode-only --update" "still not executable"
fi
...
if [ -x "$EXECBIT_DST" ]; then
  pass "s3b: session-start.sh executable after combined content+mode --update"
else
  fail "s3b: session-start.sh executable after combined content+mode --update" "still not executable"
fi
