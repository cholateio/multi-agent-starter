...
1. 探索 codebase，理解這個專案：
   - 用 ls / Read / Glob 看檔案結構
   - 讀 package.json / pyproject.toml / Cargo.toml / go.mod 等 manifest 檔（如果存在）了解 stack
   - 讀 README.md / docs/ 等已有文件了解專案目的
   - 讀幾個關鍵的 entry point（main.py、index.ts、app/page.tsx 等）了解架構

2. 基於探索結果，幫我填寫 CLAUDE.md 的前半段：
   - [PROJECT NAME]
   - Project goal（一段話描述）
   - Stack（語言、框架、資料庫、build/test 工具）
   - File layout（簡化版的 tree）

⚠️ 重要約束：
- 不要修改 CLAUDE.md 後半段的 "Multi-Agent Workflow Rules" 段落
- "Project-specific constraints" 段保留 placeholder，不要試圖填（那段需要更深的探索，下一步處理）
- 不要修改任何 production 檔案（這次只是探索）
- 如果某些資訊真的看不出來（例如「這個專案的目標」），用合理的猜測並標註「(AI 推論，請確認)」讓我事後 review

填完後告訴我：
- 你填了什麼
- 哪些地方你不確定、需要我確認
- 你看到 codebase 哪些有趣或值得記錄的特徵
```

## Step 4：AI Onboarding（**不要跳過**）

```
Onboarding 階段二：產出簡版 AI_ONBOARDING.md

⚠️ 重要：這份文件的目的是「未來開發時 AI 需要的 context」，
不是「這個專案的完整文件」。請嚴格遵守 minimalism：

只記錄會影響「日常 code 改動決策」的東西：

1. 高層架構（3-5 句話描述模組分工）
2. 主要 entry points 和 data flow（不需鉅細靡遺，能讓 AI 找到方向就好）
3. 我觀察到的 coding conventions（命名、錯誤處理、檔案組織）
4. 測試策略（怎麼跑測試、覆蓋率大概如何）
5. Build / dev workflow（npm run X / make Y 之類）

⚠️ 不要記錄：
- 環境配置細節（GCS bucket、Supabase schema、API key 位置等）
  ─ 這些「需要時再問」即可，不要預先文件化
- 你不確定的東西（不要列「我猜這是 X，請確認」清單）
  ─ 如果不確定就略過，未來真的踩到再說
- 特定功能的實作細節（除非該功能是核心架構的一部分）

⚠️ 篇幅控制：
- 整份文件目標 <100 行
- 如果寫超過，回頭刪掉「nice to have」的東西

⚠️ 確認事項上限：3 個
- 只問「不問清楚就無法做出第一個合理改動」的問題
- 例如「測試怎麼跑」是好問題（影響每次改動）
- 例如「Supabase schema 長什麼樣」是壞問題（除非我要改 DB）

⚠️ 不要修改任何 production 檔案。
```
