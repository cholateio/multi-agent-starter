...
## Review log — Round 2(final review,codex,2026-07-04)

Verdict **With fixes**,處置:

- **P1 settings.json 啟用路徑雙層巢狀**(繼承自 v3.1/v3.2 的老 bug):disabled
  塊內層多包一個 `"hooks"`,照 README「改名成 hooks」會變成 `hooks.hooks.*`,
  全部 hook 靜默失效。→ **已修**:disabled 塊直接放 event keys;hooks-smoke
  加 h0 shape 斷言(rename 後不會雙層巢狀)。
- **P2 working_tree_hash 漏 tracked-but-gitignored 檔**:空 index + `git add -A`
  會把已追蹤但被 ignore 的檔當 untracked 濾掉,fast path 對其修改失明。
  → **已修**:先 `git read-tree HEAD` 種入 throwaway index(兩份複本同步);
  hooks-smoke 加 h2o 回歸測試。
- **P2 marker 為布林、不綁定內容**(審完→再改→stop,後改的部分搭 marker 便車):
  → **記錄為有意取捨,不改機制**。理由:(a)「審完→修 findings→touch marker→停」
  是標準流程,修復內容本來就搭原審查的信任邊界(與人類 review 慣例相同);
  (b) content-bound marker 會弄壞文件明載的手動 `touch` 逃生口;(c) 對策改放在
  紀律層:/kit-review skill 明文「大波修改請重跑 review」,hook 註解載明語義。
