...
改成（跳過 shebang、印連續 `#` 檔頭、遇第一個非 `#` 行即停；**用 sed 不用
awk**——smoke 的隔離 TOOLBIN 工具清單沒有 awk，awk 會 127。已實測輸出 25 行、
止於「run init.sh <dir>.」）：
...
**Files:**
- Modify: `init.sh`（kit-owned 複製迴圈之後、`echo "updated:"` 之前插入 deploy-if-absent）
...
**插入位置關鍵**（codex plan review finding #1）：`updated:` 清單在
`init.sh` 的 `echo "updated:"`（169 行附近）就印出，所以這個區塊必須放在
**kit-owned 目錄複製迴圈結束之後、`echo "updated:"` 之前**——放到後面的
settings diff 檢查區才 append `updated_list` 的話，`+ .claude/settings.json`
永遠印不出來：
...
### Task 6: runtime 設定去 gemini——session-start.sh + settings.json（TDD）

**Files:**
- Modify: `.claude/hooks/session-start.sh`（TOOLS 區塊，`if [[ "$PROFILE" == "full" ]]` 錨點；檔頭註解 `reviewer/researcher availability`）
- Modify: `.claude/settings.json`（gemini_exec allow 行）
- Test: `tests/hooks-smoke.sh`（H1 加 KIT_CONTEXT 斷言——spec §3.6 要求，codex plan review finding #3）

- [ ] **Step 1: 寫失敗測試——hooks-smoke H1 加 KIT_CONTEXT 斷言**

full-profile 的 CTX 斷言區（`assert_contains "h1: context carries skip marker path" ...` 之後）加：
...
solo 斷言區（`assert_contains "h1: solo profile announced" ...` 之後）加：

```bash
assert_not_contains "h1: solo context has no researcher mention" "$CTX" "researcher"
```

Run: `tests/hooks-smoke.sh 2>&1 | grep '^FAIL'`
Expected: `h1: full context has no researcher field`、`h1: full context has no gemini`、
`h1: solo context has no researcher mention` 三條 FAIL（現行輸出含
`researcher=gemini(...)` 與 `reviewer/researcher`）；`reviewer=codex(` 本來就綠，
留作回歸防護。

- [ ] **Step 2: session-start.sh 簡化 TOOLS 廣播**

`if [[ "$PROFILE" == "full" ]]` 區塊改為（researcher 一項移除——研究已內建於
Claude，無外部可用性可言；solo 文案的 `reviewer/researcher` 同步改 `reviewer`）：
...
檔頭註解 `reviewer/researcher availability` 改為 `reviewer availability`。

- [ ] **Step 3: settings.json 移除 gemini_exec allow 行**

刪除 `"Bash(./.claude/scripts/gemini_exec.sh *)",` 一行。

- [ ] **Step 4: 跑兩套測試確認通過**

Run: `tests/hooks-smoke.sh && tests/smoke.sh`
Expected: 皆 `failed 0`（含 Step 1 新斷言轉綠；smoke 的 s11 cmp 對照 kit
模板自動一致）。

- [ ] **Step 5: Commit**

```bash
git add .claude/hooks/session-start.sh .claude/settings.json tests/hooks-smoke.sh
