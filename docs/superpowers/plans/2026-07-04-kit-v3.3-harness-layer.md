...
**Architecture:** SessionStart hook 記錄 session 起點的 git baseline 並向 Claude 廣播動態 context(profile、marker 路徑);Stop hook 用 baseline 補上 commit 盲區並在 gate 滿足時推進 baseline;修飾語變成正式 skills(/kit-review、/kit-skip-review),marker 的讀寫因此有了唯一的家。classify-task 只留明確覆寫,啟發式判斷還給模型。
...
- Marker/baseline 檔一律 `/tmp/claude-*-<session_id>` 命名(沿用現行慣例):
  - baseline: `/tmp/claude-kit-baseline-<session_id>`(NEW)
...
- 新增檔案全部落在 `.claude/{hooks,skills,agents}/` 之下 → install(整棵 .claude no-clobber)與 `--update`(KIT_OWNED_DIRS)自動涵蓋,**init.sh 不需要改**。
...
修法核心:SessionStart hook 記 baseline(session 起點 HEAD),Stop hook 聯集「porcelain 未 commit」+「baseline..HEAD 已 commit」;gate 滿足(無業務變更/有 marker/user bypass)時推進 baseline;porcelain 解析改用 `sed`(strip 3 字元前綴 + 取 ` -> ` 後段)。
...
**Interfaces:**
- Produces: baseline 檔 `/tmp/claude-kit-baseline-<session_id>`,內容為單行 HEAD sha 或字面 `unborn`(Task 2 的 Stop hook 讀它)。
...
- Produces: 測試 harness 函式 `pass/fail/assert_eq/assert_contains/assert_not_contains/assert_file_exists/assert_file_absent/run_hook/make_repo`(Task 2、3 的測試情境沿用)。
...
assert_eq "h1: baseline content is session-start HEAD" "$(cat "$BASELINE1")" "$HEAD1"

# write-if-missing: resume/compact must not move an existing baseline
echo "SENTINEL" > "$BASELINE1"
...
Expected: 多筆 `FAIL h1: ...`(session-start.sh 不存在 → run_hook 對不存在的檔跑 bash → 非零/空輸出),summary `failed > 0`,exit 1。
...
#   2. Record the git baseline (HEAD at session start) that
#      verify-final-review.sh diffs against — closing the "committed changes
#      are invisible to git status" blind spot in the Stop gate.
#
...
if [[ ! -f "$BASELINE_FILE" ]] && cd "$CWD" 2>/dev/null \
   && git rev-parse --git-dir >/dev/null 2>&1; then
    git rev-parse HEAD >"$BASELINE_FILE" 2>/dev/null || echo "unborn" >"$BASELINE_FILE"
fi
...
### Task 2: `verify-final-review.sh` 三 bug 修復
...
**Interfaces:**
- Consumes: Task 1 的 baseline 檔格式(單行 sha 或 `unborn`;檔案不存在 = 降級為 porcelain-only)。
- Produces: block JSON `{decision:"block", reason:...}`;reason 內含 marker 確切路徑與 `/kit-review` 指引(Task 4 的 skill 與此訊息互相呼應)。
- Produces: gate 滿足時推進 baseline(寫入當前 HEAD)並消耗 marker/bypass 檔。
...
git_f "$R2" rev-parse HEAD > "$BASELINE2"

# (a) clean tree, current baseline -> allow silently
run_hook "$VF" "$STOP_JSON"
assert_eq "h2a: clean tree allows stop" "$CODE" "0"
assert_eq "h2a: no block JSON emitted" "$OUT" ""

# (b) uncommitted business file, no marker -> block
...
# (c) marker present -> allow, marker consumed, baseline advanced
touch "$CODEX_M2"
run_hook "$VF" "$STOP_JSON"
assert_eq "h2c: marker satisfies gate" "$CODE" "0"
assert_eq "h2c: no block JSON" "$OUT" ""
assert_file_absent "h2c: marker consumed" "$CODEX_M2"
assert_eq "h2c: baseline advanced to HEAD" "$(cat "$BASELINE2")" "$(git_f "$R2" rev-parse HEAD)"

# (d) THE COMMIT BLIND SPOT: commit the business file (tree clean), baseline
# still points at pre-commit HEAD -> must still block
git_f "$R2" rev-parse HEAD > "$BASELINE2"
...
# (e) review it, marker, allow -> baseline advances -> next stop is quiet
touch "$SELF_M2"
run_hook "$VF" "$STOP_JSON"
assert_eq "h2e: marker satisfies gate after commit" "$OUT" ""
assert_eq "h2e: baseline advanced past the commit" "$(cat "$BASELINE2")" "$(git_f "$R2" rev-parse HEAD)"
run_hook "$VF" "$STOP_JSON"
...
git_f "$R2" mv renamed.py app.py   # restore for later scenarios

# (g) trivial-only change -> allow
echo "docs tweak" >> "$R2/README.md"
run_hook "$VF" "$STOP_JSON"
assert_eq "h2g: docs-only change allows stop" "$OUT" ""
git_f "$R2" checkout -q -- README.md

# (h) bypass flag -> allow + consumed + baseline advanced
echo "print('y')" > "$R2/app.py"
git_f "$R2" rev-parse HEAD > "$BASELINE2"
touch "$BYPASS2"
run_hook "$VF" "$STOP_JSON"
assert_eq "h2h: bypass allows stop" "$OUT" ""
assert_file_absent "h2h: bypass flag consumed" "$BYPASS2"
git_f "$R2" checkout -q -- app.py

# (i) stop_hook_active -> allow unconditionally (loop guard), baseline NOT advanced
echo "print('z')" > "$R2/app.py"
echo "STALE" > "$BASELINE2"
run_hook "$VF" "{\"session_id\":\"${SID2}\",\"cwd\":\"${R2}\",\"stop_hook_active\":true}"
assert_eq "h2i: stop_hook_active allows" "$OUT" ""
assert_eq "h2i: loop-guard exit does not touch baseline" "$(cat "$BASELINE2")" "STALE"
git_f "$R2" checkout -q -- app.py
rm -f "$BASELINE2"

# (j) degraded mode: no baseline file, committed change, clean tree -> allow
# (documents the porcelain-only fallback when SessionStart hook is disabled)
run_hook "$VF" "$STOP_JSON"
assert_eq "h2j: no baseline + clean tree degrades to allow" "$OUT" ""

# (k) solo profile block message discloses reduced isolation
echo "print('s')" > "$R2/app.py"
git_f "$R2" rev-parse HEAD > "$BASELINE2"
run_hook "$VF" "$STOP_JSON" solo
REASON="$(printf '%s' "$OUT" | jq -r '.reason // ""' 2>/dev/null)"
assert_contains "h2k: solo block mentions isolation OFF" "$REASON" "ISOLATION IS OFF"
assert_contains "h2k: solo block names self marker" "$REASON" "$SELF_M2"

# (l) filename with a space survives the porcelain parse
echo "x" > "$R2/my app.py"
run_hook "$VF" "$STOP_JSON"
REASON="$(printf '%s' "$OUT" | jq -r '.reason // ""' 2>/dev/null)"
assert_contains "h2l: space-containing filename listed intact" "$REASON" "my app.py"
rm -f "$R2/my app.py" && git_f "$R2" checkout -q -- app.py
rm -f "$BASELINE2"
```
...
Run: `bash tests/hooks-smoke.sh`
Expected: `h2d`(commit 盲區)、`h2b`(codex marker 路徑、/kit-review)、`h2f`(rename)、`h2l`(空白檔名)、`h2c/h2e`(baseline 推進)FAIL;h1 維持 PASS。
...
#   - uncommitted changes (git status --porcelain), PLUS
#   - commits made since the session-start baseline recorded by
#     session-start.sh (/tmp/claude-kit-baseline-<session_id>).
#   Without a baseline file (SessionStart hook disabled, no jq at session
#   start, ...) the gate degrades to the porcelain-only view.
#
# Behavior:
#   - Trivial changes (UI, docs, configs only) → allow stop
#   - Business logic changed AND a review marker exists → allow stop,
#     consume the marker, advance the baseline to current HEAD
#   - Business logic changed AND no review marker → BLOCK stop
#   - User bypass flag present → allow stop (honor user), advance baseline
#
...
# git's canonical empty-tree object — lets us diff "since before the first
# commit" when the session started on an unborn branch.
EMPTY_TREE="4b825dc642cb6eb9a060e54bf8d69288fbee4904"

# advance_baseline: called ONLY on gate-satisfied exits (nothing to review /
# reviewed / user bypass), so commits certified by this stop don't re-trigger
# the gate at the next one.
advance_baseline() {
...
# Uncommitted (staged + unstaged + untracked). Porcelain v1 lines are
# "XY path"; strip the 3-char prefix. Renames are "XY old -> new"; keep the
# new path. (A filename that itself contains " -> " would be mangled —
# acceptable for a best-effort gate.)
UNCOMMITTED=$(git status --porcelain 2>/dev/null | sed -e 's/^...//' -e 's/^.* -> //')

# Committed since the session-start baseline, if one was recorded
COMMITTED=""
if [[ -f "$BASELINE_FILE" ]] && git rev-parse --verify HEAD >/dev/null 2>&1; then
    BASE=$(head -n1 "$BASELINE_FILE" 2>/dev/null || echo "")
    [[ -z "$BASE" || "$BASE" == "unborn" ]] && BASE="$EMPTY_TREE"
    # A stale/garbage baseline (rebase, gc, tampering) must not kill the
    # hook — it just degrades back to the porcelain-only view.
...
git commit -m "fix: close the Stop-gate loop — commit blind spot, marker instructions, rename-safe parse (v3.3)"
```
...
**Interfaces:**
- Produces: 只剩兩種 `TASK_CLASSIFICATION` 值:`explicit_skip`、`explicit_full`(Task 6 的 kit-workflow.md 敘述與此對齊)。無明確修飾語時零輸出。
...
ct_ctx() {  # $1 = prompt json string value (already escaped-safe plain text)
...
Run: `bash tests/hooks-smoke.sh`
Expected: `h3: bug-fix/UI/refactor/feature ... no longer classified` 與 `.user_input` 情境 FAIL(現行版有啟發式分支、只讀 `.prompt`);h1/h2 維持 PASS。
...
# one thing grep IS reliable at: the user's explicit override phrases, in
# either language.
#
...
# Explicit opt-out — user said to skip the ceremony
...
Run: `bash tests/smoke.sh` → skill/agent 兩~三筆 FAIL(session-start.sh 已存在故 PASS;注意:enumerate_kit_files 走 git ls-files,**新檔要先 `git add` 才會被 install 鋪出**——Task 1 已 commit,此處 FAIL 的是尚未存在的 skills/agent)。
...
(取代後 `_permissionsExample_copy_to_real_permissions_if_desired` 段落消失——它畢業成真的 `permissions` 了。)
...
```
Expected: jq parse OK、smoke 全 PASS。
...
- `verify-final-review.sh`:結束前若有未審的業務邏輯就 block——v3.3 起連「已經 commit 的變更」也看得到(靠 session-start 記的 baseline),不再是只看 working tree 的半盲 gate。
...
1. **Review gate 修成真的閉環。** 三個洞:(a) marker 無人寫——hook 檢查
   ...
   (c) rename/空白檔名解析錯誤。修法:SessionStart hook 記 session 起點的
   baseline HEAD,Stop hook 聯集「未 commit + baseline 以來的 commits」,
   gate 滿足時推進 baseline;block 訊息明確給 marker 路徑與 `/kit-review`
   指引。
...
3. **修飾語 skills 化。** `/kit-review`(依 profile 跑對的 review + touch
   ...
   啟發式(button→small、refactor→large)判斷力不如模型本身,刪。
...
kit-workflow.md 同步瘦身成 landmines-only(~55 行):留下的是模型自己推不出
...
- 型別/名稱一致性:marker 與 baseline 路徑、`solo-reviewer`、`/kit-review`、`TASK_CLASSIFICATION` 值在 T1–T6 間交叉引用均已對齊。
- 已知取捨:(a) porcelain 解析對含 ` -> ` 的檔名仍會誤切(註解已載明);(b) baseline 檔在 /tmp,重開機後 resume 的 session 降級為 porcelain-only(可接受);(c) hooks 預設仍 commented-out(維持 opt-in 哲學,不在核准範圍內改變)。
