# Multi-Agent Starter Kit (v4.9.0)
...
| [`ARCHITECTURE.md`](ARCHITECTURE.md) | 想深入 | 為什麼這樣設計、v1→v4.9.0 完整取捨、四維 isolation |
...
*版本:**v4.9.0**(Stop gate 決策點化——破檻只攔一次,review/defer/skip 三選一,skip 決策權移交模型並留審計,敏感地板保留;prose 減法——audit 首跑+藉口表降級 on-demand,rules/ −4.9%;kit-evolution 新增減法紀律)。完整版本演進見 [`ARCHITECTURE.md §二`](ARCHITECTURE.md)。*
