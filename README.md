# Multi-Agent Starter Kit (v4.10.0)
...
| [`ARCHITECTURE.md`](ARCHITECTURE.md) | 想深入 | 為什麼這樣設計、v1→v4.10.0 完整取捨、四維 isolation |
...
*版本:**v4.10.0**(驗證閉環——verifier-pipe guard 物理攔「管道吞測試 exit code 再 commit」;verification-signals 擴至八節:S6 驗證器先紅過才可信/S7 驗證終點=user 可見面/S8 不可逆四要件;收據=全機隊 13 專案 85 條 LESSONS 聚類)。完整版本演進見 [`ARCHITECTURE.md §二`](ARCHITECTURE.md)。*
